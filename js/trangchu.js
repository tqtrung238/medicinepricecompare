window.onload = function () {
	...
	var tags = ["Thuốc hạ sốt", "Thuốc giảm đau", "Nước muối sinh lí", "Test covid"];
	for (var t of tags) addTags(t, "index.html?search=" + t.replace(/\s/g, '+'));
	...
	if (filters.length) { // có filter
		...
		addKhungSanPham('THUỐC', yellow_red, ['promo=thuoc', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('SỨC KHỎE SINH SẢN', blue, ['promo=sinhsan', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('THỰC PHẨM CHỨC NĂNG', yellow_red, ['promo=chucnang', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('THIẾT BỊ Y TẾ', green, ['promo=thietbi', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('CHĂM SÓC CÁ NHÂN', yellow_red, ['promo=canhan'], soLuong, div);
		addKhungSanPham('CHĂM SÓC SẮC ĐẸP', green, ['promo=sacdep'], soLuong, div);
	}
